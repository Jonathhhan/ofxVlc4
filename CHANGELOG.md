...
- **phase 3 callback boundary cleanup** — removed callback-facing static event/dialog declarations from `src/core/ofxVlc4.h`; `VlcEventRouter` now owns callback dispatch and forwards to internal instance handlers while dropping late event/dialog callbacks during shutdown; added focused router regression coverage in `tests/test_event_router.cpp` for attach/detach wiring and shutdown drop behavior
