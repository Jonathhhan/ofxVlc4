...
public:
	...
	/// Lightweight proxies providing typed, discoverable access to subsets of
	/// the ofxVlc4 API.  The flat API remains the canonical full interface.
	/// @{
	...
	/// @brief Access audio-related functionality.
	AudioModule & audio();
	const AudioModule & audio() const;

	/// @brief Access video-related functionality.
	VideoModule & video();
	const VideoModule & video() const;

	/// @brief Access media / playlist functionality.
	MediaModule & media();
	const MediaModule & media() const;

	/// @brief Access recording-related functionality.
	RecordingModule & recording();
	const RecordingModule & recording() const;

	/// @brief Access MIDI-related functionality.
	MidiModule & midi();
	const MidiModule & midi() const;

	/// @}

	/// @brief Get the current playback state as a typed enum.
	PlaybackState getPlaybackState() const;

	/// @name Builder
	/// @{

	/// Fluent builder for constructing and initialising an ofxVlc4 instance.
	class Builder {
	public:
		Builder() = default;

		Builder & logLevel(ofLogLevel level);
		Builder & subtitleFont(const std::string & fontFamily);
		Builder & subtitleTextScale(float scale);
		Builder & subtitleRenderer(ofxVlc4SubtitleTextRenderer renderer);
		Builder & audioCaptureEnabled(bool enabled);
		Builder & audioCaptureSampleRate(int rate);
		Builder & audioCaptureChannelCount(int channels);
		Builder & audioCaptureBufferSeconds(double seconds);
		Builder & audioMixMode(AudioMixMode mode);
		Builder & videoOutputBackend(VideoOutputBackend backend);
		Builder & extraArg(const std::string & arg);
		Builder & extraArgs(const std::vector<std::string> & args);

		/// @brief Build and initialise the player.
		/// @return A fully-configured, init()-ed ofxVlc4 instance.
		std::unique_ptr<ofxVlc4> build() const;

	private:
		ofLogLevel m_logLevel = OF_LOG_NOTICE;
		std::string m_subtitleFont;
		float m_subtitleTextScale = 1.0f;
		ofxVlc4SubtitleTextRenderer m_subtitleRenderer = ofxVlc4SubtitleTextRenderer::Auto;
		bool m_audioCaptureEnabled = true;
		int m_audioCaptureSampleRate = 44100;
		int m_audioCaptureChannelCount = 2;
		double m_audioCaptureBufferSeconds = 0.75;
		AudioMixMode m_audioMixMode = AudioMixMode::Auto;
		VideoOutputBackend m_videoOutputBackend = VideoOutputBackend::Texture;
...
public:
	...
	double getMidiPositionFraction() const;
	void seekMidiFraction(double fraction);
	void setMidiFinishedCallback(MidiFinishedCallback callback);
	void clearMidiFinishedCallback();
	bool hasMidiFinishedCallback() const;
