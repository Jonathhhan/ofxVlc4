...
bool MediaLibrary::isPlaylistIndexLocked(int index) const {
	return index >= 0 && index < static_cast<int>(owner.playlist.size());
}

std::string MediaLibrary::getPathAtIndexLocked(int index) const {
	return isPlaylistIndexLocked(index) ? owner.playlist[static_cast<size_t>(index)] : "";
}

std::string MediaLibrary::getCurrentPathLocked() const {
	return getPathAtIndexLocked(owner.currentIndex);
}
...
size_t MediaLibrary::getPlaylistSizeLocked() const {
	return owner.playlist.size();
}

bool MediaLibrary::hasPlaylistLocked() const {
	return !owner.playlist.empty();
}
...
void MediaLibrary::addToPlaylistInternal(const std::string & path, bool preloadMetadata) {
	...
	{
		std::lock_guard<std::mutex> lock(owner.metadataCacheMutex);
		owner.metadataCache.erase(path);
	}
...
void MediaLibrary::resetCurrentMediaParseState() {
	owner.currentMediaParseStatus.store(static_cast<int>(ofxVlc4::MediaParseStatus::None));
	owner.lastCompletedMediaParseStatus.store(static_cast<int>(ofxVlc4::MediaParseStatus::None));
	owner.mediaParseRequested.store(false);
	owner.mediaParseActive.store(false);
}
...
std::vector<ofxVlc4::PlaylistItemInfo> MediaLibrary::getPlaylistItems() const {
	...
	{
		std::lock_guard<std::mutex> lock(owner.playlistMutex);
		paths = owner.playlist;
		currentIndex = owner.currentIndex;
	}

	std::vector<ofxVlc4::PlaylistItemInfo> items;
	items.reserve(paths.size());

	std::lock_guard<std::mutex> metadataLock(owner.metadataCacheMutex);
	for (size_t i = 0; i < paths.size(); ++i) {
		...
		item.uri = isUri(path);
		item.metadataCached = owner.metadataCache.find(path) != owner.metadataCache.end();
...
ofxVlc4::PlaylistStateInfo MediaLibrary::getPlaylistStateInfo() const {
	...
	{
		std::lock_guard<std::mutex> lock(owner.playlistMutex);
		state.currentIndex = owner.currentIndex;
	}
...
void MediaLibrary::clearGeneratedThumbnailInfo() {
	std::lock_guard<std::mutex> lock(owner.thumbnailMutex);
	owner.lastGeneratedThumbnail = ofxVlc4::ThumbnailInfo {};
}

void MediaLibrary::clearMetadataCache() {
	std::lock_guard<std::mutex> lock(owner.metadataCacheMutex);
	owner.metadataCache.clear();
}
...
void MediaLibrary::cacheArtworkPathForMediaPath(const std::string & mediaPath, const std::string & artworkPath) {
	...
	std::lock_guard<std::mutex> lock(owner.metadataCacheMutex);
	auto & metadata = owner.metadataCache[currentPath];
...
std::vector<std::pair<std::string, std::string>> MediaLibrary::getMetadataAtIndex(int index) const {
	...
	if (index == getCurrentIndex() && currentMedia) {
		...
		if (!currentMediaMetadata.empty()) {
			std::lock_guard<std::mutex> lock(owner.metadataCacheMutex);
			const auto cached = owner.metadataCache.find(path);
			if (cached != owner.metadataCache.end() &&
			...
			owner.metadataCache[path] = currentMediaMetadata;
...

	{
		...
		if (cached == owner.metadataCache.end() ||
			...
			owner.metadataCache[path] = metadata;
		}
	}
...
ofxVlc4::MediaParseInfo MediaLibrary::getCurrentMediaParseInfo() const {
	...
	info.status = static_cast<ofxVlc4::MediaParseStatus>(owner.currentMediaParseStatus.load());
	info.lastCompletedStatus = static_cast<ofxVlc4::MediaParseStatus>(owner.lastCompletedMediaParseStatus.load());
	info.requested = owner.mediaParseRequested.load();
	info.active = owner.mediaParseActive.load();
...
ofxVlc4::MediaParseOptions MediaLibrary::getMediaParseOptions() const {
	return owner.mediaParseOptions;
}

void MediaLibrary::setMediaParseOptions(const ofxVlc4::MediaParseOptions & options) {
	owner.mediaParseOptions.parseLocal = options.parseLocal;
	owner.mediaParseOptions.parseNetwork = options.parseNetwork;
	owner.mediaParseOptions.forced = options.forced;
	owner.mediaParseOptions.fetchLocal = options.fetchLocal;
	owner.mediaParseOptions.fetchNetwork = options.fetchNetwork;
	owner.mediaParseOptions.doInteract = options.doInteract;
	owner.mediaParseOptions.timeoutMs = std::max(-1, options.timeoutMs);
}
...
bool MediaLibrary::requestCurrentMediaParse() {
	...
	const int result = libvlc_media_parse_request(
		instance,
		sourceMedia,
		toLibvlcMediaParseFlags(owner.mediaParseOptions),
		owner.mediaParseOptions.timeoutMs);
	...
	owner.currentMediaParseStatus.store(static_cast<int>(ofxVlc4::MediaParseStatus::Pending));
	owner.mediaParseRequested.store(true);
	owner.mediaParseActive.store(true);
...
void MediaLibrary::handleMediaParsedChanged(libvlc_media_parsed_status_t status) {
	...
	owner.currentMediaParseStatus.store(static_cast<int>(publicStatus));
	owner.mediaParseRequested.store(false);
	owner.mediaParseActive.store(publicStatus == ofxVlc4::MediaParseStatus::Pending);
	if (publicStatus == ofxVlc4::MediaParseStatus::Skipped ||
		...
		owner.lastCompletedMediaParseStatus.store(static_cast<int>(publicStatus));
	}
...
ofxVlc4::ThumbnailInfo MediaLibrary::getLastGeneratedThumbnail() const {
	std::lock_guard<std::mutex> lock(owner.thumbnailMutex);
	return owner.lastGeneratedThumbnail;
}
...
bool MediaLibrary::requestThumbnailByTime(
	...
	{
		std::lock_guard<std::mutex> lock(owner.thumbnailMutex);
		owner.thumbnailRequest = request;
		owner.lastGeneratedThumbnail.requestActive = true;
	}
...
void MediaLibrary::cancelThumbnailRequest() {
	std::lock_guard<std::mutex> lock(owner.thumbnailMutex);
	if (owner.thumbnailRequest) {
		libvlc_media_thumbnail_request_destroy(owner.thumbnailRequest);
		owner.thumbnailRequest = nullptr;
	}
	owner.lastGeneratedThumbnail.requestActive = false;
}
...
void MediaLibrary::handleThumbnailGenerated(libvlc_picture_t * picture) {
	...
	owner.lastGeneratedThumbnail.available = true;
	owner.lastGeneratedThumbnail.path = thumbnailPath;
	owner.lastGeneratedThumbnail.timeMs = libvlc_picture_get_time(picture);
	owner.lastGeneratedThumbnail.width = libvlc_picture_get_width(picture);
	owner.lastGeneratedThumbnail.height = libvlc_picture_get_height(picture);
}
...
void MediaLibrary::updateSnapshotStateOnRequest(const std::string & requestedPath) {
	...
	owner.snapshotPending = !trimmedPath.empty();
	owner.snapshotAvailable = !owner.lastSnapshotPath.empty();
	owner.pendingSnapshotPath = trimmedPath;
	owner.lastSnapshotEventMessage = trimmedPath.empty() ? "" : ("Requested: " + trimmedPath);
	owner.lastSnapshotFailureReason.clear();
}
...
void MediaLibrary::updateSnapshotStateFromEvent(const std::string & savedPath) {
	std::string resolvedPath;
	{
		...
		if (resolvedPath.empty()) {
			resolvedPath = owner.pendingSnapshotPath;
		}
		owner.snapshotPending = false;
		owner.pendingSnapshotPath.clear();
		owner.snapshotAvailable = !resolvedPath.empty();
		if (!resolvedPath.empty()) {
			owner.lastSnapshotPath = resolvedPath;
			owner.lastSnapshotBytes = fileSizeIfAvailable(resolvedPath);
			owner.lastSnapshotTimestamp = ofGetTimestampString("%Y-%m-%d %H:%M:%S");
			owner.lastSnapshotEventMessage = "Saved: " + resolvedPath;
			owner.lastSnapshotFailureReason.clear();
		}
	}
...
void MediaLibrary::updateSnapshotFailureState(const std::string & failureReason) {
	...
	owner.snapshotAvailable = !owner.lastSnapshotPath.empty();
	owner.pendingSnapshotPath.clear();
	owner.lastSnapshotFailureReason = trimmedReason;
	owner.lastSnapshotEventMessage = trimmedReason.empty() ? "" : ("Failed: " + trimmedReason);
}
...
std::vector<ofxVlc4::BookmarkInfo> MediaLibrary::getBookmarksForPath(const std::string & path) const {
	...
	{
		std::lock_guard<std::mutex> lock(owner.bookmarkMutex);
		const auto it = owner.bookmarksByPath.find(trimmedPath);
		if (it == owner.bookmarksByPath.end()) {
			return {};
		}
		bookmarks = it->second;
	}
...
bool MediaLibrary::addBookmarkAtTime(int timeMs, const std::string & label) {
	...
	{
		std::lock_guard<std::mutex> lock(owner.bookmarkMutex);
		auto & bookmarks = owner.bookmarksByPath[currentPath];
...
bool MediaLibrary::seekToBookmark(const std::string & bookmarkId) {
	...
	{
		std::lock_guard<std::mutex> lock(owner.bookmarkMutex);
		const auto it = owner.bookmarksByPath.find(currentPath);
		if (it == owner.bookmarksByPath.end()) {
			return false;
		}
...
bool MediaLibrary::removeBookmark(const std::string & bookmarkId) {
	...
	if (it->second.empty()) {
		owner.bookmarksByPath.erase(it);
	}
...
void MediaLibrary::clearBookmarksForPath(const std::string & path) {
	...
	std::lock_guard<std::mutex> lock(owner.bookmarkMutex);
	owner.bookmarksByPath.erase(trimmedPath);
}
