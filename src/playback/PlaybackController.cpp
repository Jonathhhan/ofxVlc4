...
void PlaybackController::processDeferredPlaybackActions() {
	...
	if (playbackTransport.manualStopInProgress.load()) {
		const libvlc_state_t state = player ? libvlc_media_player_get_state(player) : libvlc_Stopped;
		const auto manualStopDecision = ofxVlc4MediaHelpers::evaluateDeferredManualStop(
			...
			player != nullptr,
			!player || isStoppedOrIdleState(state),
			playbackTransport.manualStopRequestTimeMs.load(),
			ofGetElapsedTimeMillis(),
			playbackTransport.manualStopRetryIssued.load(),
			kManualStopRetryMs,
			kManualStopFallbackMs);

		if (manualStopDecision.shouldResetManualStop) {
			playbackTransport.manualStopInProgress.store(false);
			playbackTransport.pendingManualStopEvents.store(0);
			playbackTransport.manualStopRequestTimeMs.store(0);
			playbackTransport.manualStopRetryIssued.store(false);
		...
		if (manualStopDecision.shouldFinalizeManualStop) {
			playbackTransport.pendingManualStopEvents.store(0);
			playbackTransport.manualStopRequestTimeMs.store(0);
			playbackTransport.manualStopRetryIssued.store(false);
...
void PlaybackController::onMediaPlayerStopped() {
	...
	const libvlc_state_t state = player ? libvlc_media_player_get_state(player) : libvlc_Stopped;
	const auto stopDecision = ofxVlc4MediaHelpers::evaluateStoppedPlaybackEvent(
		pendingManualStops,
		playbackTransport.pendingActivateIndex.load() >= 0,
	...
	if (stopDecision.keepManualStopInProgress) {
		playbackTransport.manualStopInProgress.store(true);
		if (stopDecision.shouldActivatePendingRequest) {
			playbackTransport.pendingActivateReady.store(true);
		}
		if (stopDecision.shouldFinalizeManualStop) {
			playbackTransport.pendingManualStopFinalize.store(true);
		}
	} else {
		playbackTransport.manualStopInProgress.store(false);
		playbackTransport.pendingManualStopEvents.store(0);
		if (stopDecision.shouldQueuePlaybackAdvance) {
			playbackTransport.playNextRequested.store(true);
		}
	}
}
