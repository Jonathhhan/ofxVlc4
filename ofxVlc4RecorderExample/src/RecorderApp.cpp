...
void RecorderApp::audioOut(ofSoundBuffer & buffer) {
	buffer.set(0.0f);
...
void RecorderApp::keyPressed(int key) {
	if (!player || shuttingDown) {
		return;
	}
...
void RecorderApp::dragEvent(ofDragInfo dragInfo) {
...
void RecorderApp::windowResized(int, int) {
}

void RecorderApp::shutdownPlayer() {
	if (!player || shuttingDown) {
		return;
	}
...
void RecorderApp::setupBenchmarkAudio() {
...
void RecorderApp::closeBenchmarkAudio() {
	benchmarkAudioStream.stop();
	benchmarkAudioStream.close();
}

void RecorderApp::stopActiveRecorders() {
	if (!player) {
		return;
	}
...
void RecorderApp::loadSeedMedia() {
	if (!player || shuttingDown) {
		return;
	}
...
bool RecorderApp::allocateBenchmarkTexture() {
	...
	if (!benchmarkFbo.isAllocated()) {
		ofLogError("RecorderApp") << "Failed to allocate benchmark FBO "
...
void RecorderApp::updateBenchmarkTexture() {
...
bool RecorderApp::loadMediaPath(const std::string & path, bool autoPlay) {
...
void RecorderApp::replacePlaylistFromDroppedFiles(const std::vector<std::filesystem::path> & paths) {
	if (!player) {
		return;
	}
...
void RecorderApp::updateAudioOnlyMode() {
...
bool RecorderApp::isCurrentMediaAudioOnly() const {
	if (benchmarkModeActive) {
		return false;
	}
	if (!player) {
...
void RecorderApp::toggleBenchmarkMode() {
	if (!player) {
		return;
	}
...
void RecorderApp::toggleBenchmarkAudioSource() {
	if (isBenchmarkRecordingActive()) {
		return;
	}
...
void RecorderApp::cycleVideoRecordingCodec() {
...
void RecorderApp::cycleBenchmarkMuxProfile() {
...
void RecorderApp::toggleBenchmarkMuxSourceCleanup() {
	if ((player && (player->isRecordingMuxPending() || player->isRecordingMuxInProgress())) ||
		isBenchmarkRecordingActive()) {
		return;
	}
...
void RecorderApp::toggleBenchmarkVideoRecording() {
	if (!player) {
		return;
	}
	...
	stopActiveRecorders();
	if (!benchmarkFbo.isAllocated()) {
		ofLogError("RecorderApp") << "Benchmark recording requested without an allocated FBO.";
		return;
	}
...
void RecorderApp::toggleWindowRecording() {
	if (!player) {
		return;
	}
	if (windowRecordingActive || isWindowCaptureRecordingActive()) {
...
void RecorderApp::cycleReadbackPolicy() {
	if (!player) {
		return;
	}
...
void RecorderApp::adjustReadbackBufferCount(int delta) {
	if (!player) {
		return;
	}
...
void RecorderApp::toggleNativeRecording() {
	if (!player) {
		return;
	}
...
void RecorderApp::toggleAudioRecording() {
	if (!player) {
		return;
	}
...
std::string RecorderApp::currentMediaLabel() const {
	if (isWindowCaptureRecordingActive()) {
		return "app window";
	}
	if (benchmarkModeActive) {
...
std::string RecorderApp::playbackLabel() const {
	if (isWindowCaptureRecordingActive()) {
...
bool RecorderApp::isBenchmarkRecordingActive() const {
...
bool RecorderApp::isWindowCaptureRecordingActive() const {
	return windowRecordingActive || (player && player->isWindowRecording());
}

void RecorderApp::requestBenchmarkRecordingStop() {
	if (!player) {
		return;
	}
...
void RecorderApp::requestWindowRecordingStop() {
	if (!player) {
		return;
	}
...
std::string RecorderApp::recordingSessionStateLabel() const {
	if (!player) {
		return "Closed";
	}
...
bool RecorderApp::isRecordingBusy() const {
	if (!player) {
		return false;
	}
...
void RecorderApp::drawControlPanel() {
	if (!player) {
		return;
	}
...
std::string RecorderApp::overlayStatusLine() const {
	if (!player) {
